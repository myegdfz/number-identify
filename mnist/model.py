...
# 线性模型
# usually regression model as y = k*x + b, so we define the function here same as it
def regression(x):
    K = tf.Variable(tf.zeros([784, 10]), name='K')
    b = tf.Variable(tf.zeros([10]), name='b')
    y = tf.nn.softmax(tf.matmul(x, K) + b)
    return y, [K, b]


def convolutional(x, keep_pro):
    def conv2d(x, w):
